...
CUSTOMER_NAME_FIELD = "Customer Name"  # adjust if different
STATUS_FIELD        = "Status"
...
def parse_pasted_list(raw: str) -> List[str]:
    ...
    for t in tokens:
        # For short "word word" inputs, split on spaces
...
input_ticket = (
    st.text_input("🎫 Ticket Number") if search_type == "Ticket Number" else None
)
input_invoice = (
    st.text_input("📄 Invoice Number")
    if search_type in ["Invoice Number", "Invoice + Item Pair"]
    else None
)
input_item = (
    st.text_input("📦 Item Number")
    if search_type in ["Item Number", "Invoice + Item Pair"]
    else None
)

uploaded_file = (
    st.file_uploader(
        "📤 (Optional) Upload CSV with 'Invoice Number' and 'Item Number'", type=["csv"]
    )
    if search_type == "Invoice + Item Pair"
    else None
)
...
if st.button("🔎 Search"):
    try:
        ...
        if data:
            ...
            pair_wanted = set()
            pair_mode_with_csv = (
                search_type == "Invoice + Item Pair" and uploaded_file is not None
            )
            if pair_mode_with_csv:
                pair_df = pd.read_csv(uploaded_file)
                if not {INVOICE_FIELD, ITEM_FIELD}.issubset(pair_df.columns):
                    st.error(
                        f"CSV must contain '{INVOICE_FIELD}' and '{ITEM_FIELD}' columns."
                    )
                    st.stop()
                pair_df[INVOICE_FIELD] = (
                    pair_df[INVOICE_FIELD].astype(str).str.strip().str.upper()
                )
                pair_df[ITEM_FIELD] = (
                    pair_df[ITEM_FIELD].astype(str).str.strip().str.upper()
                )
                pair_wanted = set(zip(pair_df[INVOICE_FIELD], pair_df[ITEM_FIELD]))

            # For fuzzy customer not_found
            found_tokens = set()
            ...
            for key, record in (data or {}).items():
                ...
                cust_nm = (
                    norm(record.get(CUSTOMER_NAME_FIELD, ""))
                    if CUSTOMER_NAME_FIELD in record
                    else ""
                )
                ...
                if search_type == "Ticket Number":
                ...
                elif search_type == "Multiple Tickets (paste list)":
                    ...
                    if customer_mode == "Exact":
                        if cust_no in pasted_set:
                            found_tokens.add(cust_no)
                            match = True
                        if customer_search_name and cust_nm in pasted_set:
                            found_tokens.add(cust_nm)
                            match = True
                    elif customer_mode == "Starts with":
                        for tok in pasted_set:
                            if cust_no.startswith(tok) or (
                                customer_search_name and cust_nm.startswith(tok)
                            ):
                        ...
                        for tok in pasted_set:
                            if tok in cust_no or (
                                customer_search_name and tok in cust_nm
                            ):
                                found_tokens.add(tok)
                                match = True
                                break
            ...
            # 6) Build not_found list for bulk modes
            if search_type in [
                "Multiple Tickets (paste list)",
                "Multiple Invoices (paste list)",
                "Multiple RTNs (paste list)",
                ...
                if search_type == "Multiple Invoices (paste list)":
                    ...
                    field_name = None  # customers use found_tokens
        ...
        if matches:
            ...
            # JSON view (one expander per record – no nesting)
