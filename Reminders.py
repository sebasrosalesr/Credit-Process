...
def add_reminder(ticket: str, note: str, hours: int):
    ...
    with sqlite3.connect(DB_PATH, timeout=30.0) as con:
        con.execute(
            ...
            (now.isoformat(), due_at, ticket, note),
        )
        con.commit()
...
with st.form("add_reminder_form", clear_on_submit=True):
    ...
    custom_h = st.number_input(
        "Custom hours", min_value=1, max_value=336, value=4, step=1
        ...
        with st.container(border=True):
            ...
            with c1:
                ...
                if row["note"]:
                    st.caption(row["note"])
                st.caption(
                    f"Created: {row['created_at'].tz_convert('UTC').strftime('%m-%d %H:%M')} UTC"
                )
            with c2:
                st.write("**Due**")
                st.markdown(
                    f"<span style='color:{color}; font-weight:600'>{due_label}</span>",
            ...
            with c3:
                ...
                with cs_col1:
                    cs_h = st.number_input(
                        "hrs", min_value=1, max_value=336, value=12, key=f"cs_h_{rid}"
                    )
                with cs_col2:
...
if not done_df.empty:
    ...
    st.dataframe(
        view[["ticket", "note", "created_at", "due_at"]].rename(
            columns={
                "ticket": "Ticket",
                "note": "Note",
                "created_at": "Created",
                ...
                if not uploaded:
                    ...
                    try:
                        ...
                        with st.spinner("Importing safely..."):
                            ...
                            # Step 2: Use SQLite backup API (atomic)
...
st.caption(
    "Local SQLite • No cloud • Daily export • Zero data loss • Built by Sebastian Rosales"
)
